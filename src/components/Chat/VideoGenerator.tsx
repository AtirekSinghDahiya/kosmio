...
export const VideoGenerator: React.FC<VideoGeneratorProps> = ({ onClose, initialPrompt = '' }) => {
  ...
  const [provider, setProvider] = useState<VideoProvider>('veo3');
  ...
  const handleGenerate = async (customPrompt?: string) => {
    ...
    if (provider === 'sora' && !hasAccess) {
      if (isCheckingAccess) {
        showToast('info', 'Checking Access', 'Please wait while we verify your access...');
        return;
      }
      showToast('error', 'Premium Feature', 'Sora 2 requires tokens. Please ensure you have tokens in your account.');
      return;
    }

    if (provider === 'sora' && !soraAvailable) {
      showToast('error', 'API Key Missing', 'Fal.ai API key is not configured');
      return;
    }

    if (provider === 'veo3' && !veo3Available) {
      showToast('error', 'API Key Missing', 'Fal.ai API key is not configured');
      return;
    }
                ...
                {isCheckingAccess && provider === 'sora' && (
                  <div className="flex items-start gap-3 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                    ...
                    <div className="text-sm text-blue-200">
                      ...
                      <p className="text-blue-300/80">Verifying your token balance...</p>
                    </div>
                  </div>
                )}

                {!isCheckingAccess && !hasAccess && provider === 'sora' && (
                  <div className="flex items-start gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                    <AlertCircle className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
                    <div className="text-sm text-yellow-200">
                      <p className="font-semibold mb-1">Tokens Required</p>
                      <p className="text-yellow-300/80">Sora 2 requires tokens in your account. Please purchase a token pack to access this feature.</p>
                    </div>
                  </div>
                )}
                ...
                {!isCheckingAccess && hasAccess && provider === 'sora' && (
                  <div className="flex items-start gap-3 p-4 bg-green-500/10 border border-green-500/30 rounded-xl">
                    ...
                    <div className="text-sm text-green-200">
                      ...
                      <p className="text-green-300/80">You have full access to Sora 2 video generation!</p>
                    </div>
                  </div>
                )}
